...
class ImageNetJoiner(nn.Module):
    def __init__(self, num_encoder_layers = 8, nhead=8, num_classes = 10, mask=None, pos_enc = "sin", batch_size=10, in_chans=3, hidden_dim=768, image_h=256, image_w=256, grid_l=16, gm_patch = 32, use_patches=True, attn_layer = 1,penalty_factor="1", alpha=1):
        super().__init__()
    ...
    def rescale(self, sattn):
        ...
        #attn = self.avgPool(a.reshape(bs,self.num_patches,64).permute(0, 2, 1).reshape(bs,64,self.f_map_h,self.f_map_w)).permute(0, 2, 3, 1).reshape(bs,64,64)
