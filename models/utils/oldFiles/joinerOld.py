...
class Joiner(nn.Module):
    def __init__(self, num_encoder_layers = 6, nhead=1, backbone = False, num_classes = 10, bypass=False, mask=None, pos_enc = "sin", batch_size=10, hidden_dim=256, image_h=32, image_w=32, grid_l=4, penalty_factor="1", alpha=1, pretrained=False):
        super().__init__()
        ...
        if backbone == True:
            ...
            self.backbone = Backbone(hidden_dim=hidden_dim, pretrained=self.pretrained)
        else:
    ...
    def assertParams(self):
        ...
        for name, p in self.named_parameters(): 
            if name not in fb and self.pretrained == False:
                assert p.requires_grad == True
            #elif name not in fb and self.pretrained == True:
            #    assert p.requires_grad == False
        if self.resnet_backbone == True and self.pretrained == True:
    ...
    def forward(self, inputs, mask=Optional[Tensor], pos=Optional[Tensor], penalty_mask=Optional[Tensor]):
        ...
        if self.pos_enc == "learned":
            ...
            att, sattn = self.encoder(src= 1*h, mask=mask, pos_embed=pos)
